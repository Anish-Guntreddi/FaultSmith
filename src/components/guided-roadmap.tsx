...
export function GuidedRoadmap({
  ...
  return (
    <section aria-labelledby="guided-roadmap-heading">
      <div className="mb-5 flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
        <div>
          <div className="text-xs uppercase tracking-[0.18em] text-zinc-500">Evidence-first curriculum</div>
          <h2 id="guided-roadmap-heading" className="mt-1 text-2xl font-medium text-white">Your debugging roadmap</h2>
        ...
        <div className="min-w-52 rounded-xl border border-white/8 bg-black/15 px-4 py-3">
          ...
          <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-white/6"><div className="h-full rounded-full bg-emerald-400/70 transition-[width]" style={{ width: `${(completeCount / learningSteps.length) * 100}%` }} /></div>
        </div>
      </div>
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_390px]">
        <div className="space-y-4">
          {learningPhases.map((phase) => (
            <section key={phase.id} aria-labelledby={`${phase.id}-heading`} className="rounded-2xl border border-white/9 bg-[#101318]/90 p-4 sm:p-5">
              <div className="flex gap-3">
                <span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg border border-amber-400/20 bg-amber-400/[0.06] text-xs font-semibold text-amber-300">{phase.order}</span>
                <div><h3 id={`${phase.id}-heading`} className="text-base font-medium text-zinc-100">{phase.title}</h3><p className="mt-1 text-xs leading-5 text-zinc-500">{phase.description}</p></div>
              </div>
                ...
                {phase.steps.map((step) => {
                  ...
                  return (
                    <button
                      ...
                      className={`min-h-32 rounded-xl border p-3 text-left transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-300 ${selectedLesson ? "border-amber-400/45 bg-amber-400/[0.07]" : "border-white/7 bg-black/15 hover:border-white/16"}`}
                    >
                      ...
                      {recommended && <div className="mt-2 text-[10px] font-semibold uppercase tracking-wider text-amber-300">Recommended</div>}
                    </button>
                  );
                })}
              </div>
        ...
        <aside aria-label="Selected guided lesson" className="self-start rounded-2xl border border-white/10 bg-[#111419]/95 p-5 shadow-2xl shadow-black/20 xl:sticky xl:top-5">
          <div className="flex items-center justify-between gap-3"><span className="text-xs uppercase tracking-[0.18em] text-zinc-500">Lesson {selected.order} of 9</span><span className="rounded-full border border-white/8 px-2.5 py-1 text-[10px] text-zinc-400">{selected.category}</span></div>
          <h3 className="mt-4 text-xl font-medium text-white">{selected.title}</h3>
          ...
          <div className="mt-5 rounded-xl border border-amber-400/12 bg-amber-400/[0.04] p-4">
            <div className="text-[10px] font-semibold uppercase tracking-wider text-amber-300">Concept guide</div>
            <p className="mt-2 text-xs leading-5 text-zinc-300">{selected.conceptGuide}</p>
          </div>

          <div className="mt-5">
            <div className="text-[10px] font-semibold uppercase tracking-wider text-zinc-500">Investigation loop</div>
          ...
          <div className="mt-5 rounded-xl border border-white/7 bg-black/20 p-4">
            <div className="text-[10px] uppercase tracking-wider text-zinc-500">Success signal</div>
          ...
          <button type="button" disabled={!selectedUnlocked} onClick={() => onStartStep(selected)} className="mt-4 w-full rounded-xl bg-amber-400 px-4 py-3.5 text-sm font-semibold text-[#1a1105] transition hover:bg-amber-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white disabled:opacity-35">{selectedComplete ? "Practice lesson again" : "Start guided lab"} <span aria-hidden="true">→</span></button>
