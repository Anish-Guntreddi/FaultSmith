...
**Review date:** July 20, 2026 (updated for the presentation-polish handoff)
...
| Low; mitigated | Visual redesign or explanatory motion adds a remote origin, hidden interaction, stale inline state, scroll trap, excessive layers, or accessibility regression | global styles and learning/account/workspace/report/story surfaces | system fonts and CSS geometry only; exact CSP unchanged; GSAP/ScrollTrigger are bundled split chunks dynamically loaded only near the desktop story; native sticky scroll without pin/snap/smoothing; semantic chapters precede enhancement; reduced-motion/mobile/static paths; deterministic teardown clears inline properties; one-shot/pausable motion controls; mobile disclosure dismissal and focus restoration; compositor hints limited to five layers | lint/typecheck/build; 24 standard and 16 Firebase-mode browser scenarios; axe, keyboard, password-manager, responsive, menu-dismissal, and desktop→mobile/reduced-motion transition review | presentation code adds no authority, data, credential, or network boundary |
...
## Local credential-file scan boundary — July 20, 2026

The canonical source gate now scans the exact set that can enter Git: all cached/tracked paths plus untracked paths not excluded by `.gitignore`. This replaces raw filesystem traversal at the repository root. The policy permits a real operator `.env.local` to exist for local Firebase/OpenAI verification without either printing its values or making the gate permanently unusable. It does not create a tracked-secret blind spot: any ignored file that has already been added to Git is returned through `--cached` and is scanned, and all reachable commits continue to be inspected independently.

The updated direct gate passed across 224 committable working-tree files and 89 reachable commits; the client leakage gate passed 25 artifacts; the dependency audit found zero vulnerabilities. Local credential material remains server/operator-owned, gitignored, and outside documentation, screenshots, browser state inspection, and test evidence. No credential value was printed or committed during this pass.

## In-lab account prompt boundary — July 20, 2026

The new account prompts create no identity or persistence mechanism. They render only when the existing Firebase configuration is complete and the learner is not signed in, disclose that guest data already remains on the device, and expand the existing `ProgressSyncPanel` for email/password or Google sign-in. Signed-in learners and deployments without account configuration receive no conversion prompt. No hypothesis, explanation, source file, hint, password, token, email, or UID was added to progress records or prompt state.
