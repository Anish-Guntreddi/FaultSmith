...
- The Phase 01.1 candidate scan passed across 483 working-tree files and 58 reachable commits. Only the exact `NEXT_PUBLIC_FIREBASE_API_KEY` name is allowlisted as public Firebase metadata; name variants and any publicized server secret still fail closed, and no real Firebase value exists anywhere in the repository (all suites use the fake `demo-faultsmith` emulator project).
