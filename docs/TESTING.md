...
The Firebase browser suite also includes a focused guest-conversion regression. It opens a guided lab while signed out, confirms the local-save and personalized-metrics disclosure, expands the in-place account controls, verifies email/Google actions remain reachable, runs an axe scan, checks 390 × 844 horizontal containment, completes the verified fixture repair, and confirms the post-result account prompt. The new scenario passes without contacting a real Firebase project or consuming OpenAI credits.
